...
3. ~~**Adapter + ledger.**~~ **Done 2026-08-20.** `www/platform/index.js` with
   ...
   can be checked headlessly); `tools/check-achievements.mjs`, 56 assertions at the time,
...
5. **Billing.** Choose the plugin. Wire purchase, restore, entitlement, fake ads.
   **No purchase achievement** — decided 2026-08-23, and it is arithmetic rather
   ...
   refund path
6. **Assets and release build.** ~~Icons~~ **done 2026-08-25** — adaptive launcher icon at all five densities, the 512 store icon and the 1024×500 feature graphic, all generated from `www/core/math.js` by `tools/logo/`. Splash still to do. Upload keystore
...
| 7 | Listing: feature graphic 1024×500 and the 512 store icon **both exist now** in `docs/store/`, generated by `tools/logo/`; still needed are ≥2 phone screenshots and the description | — |
